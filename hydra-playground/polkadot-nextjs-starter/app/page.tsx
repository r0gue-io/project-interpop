...
const FundOnHydra: React.FC = () => {
    ...
    const handleFund = async () => {
        ...
        try {
            ...
            console.log(`Funding contract at address: ${contractAddress} with amount: ${amount}`);

            const fundOnHydraSdk = createInkSdk(api, contracts.execute_on_hydra);
            const fund_on_hydra = fundOnHydraContract.message("create_pop_to_hydra_xcm");

            const message_data = fund_on_hydra.encode({
                amount: BigInt(amount),
                ref_time: 10000000000n, // TODO: make param
                proof_size: 1000000n
            });
            const contract = fundOnHydraSdk.getContract(contractAddress);

            console.log(selectedAccount?.address as SS58String);
            let res = await contract.send("create_pop_to_hydra_xcm", {
    ...
    return (
        <div style={{ padding: '16px', border: '1px solid #ccc', borderRadius: '8px', maxWidth: '400px' }}>
            ...
            <button
                onClick={handleFund}
                disabled={isLoading || !contractAddress || !amount || parseFloat(amount) <= 0}
                style={{
                    width: '100%',
                    padding: '10px',
                    ...
                    border: 'none',
                    borderRadius: '4px',
                    cursor: isLoading || !contractAddress || !amount || parseFloat(amount) <= 0 ? 'not-allowed' : 'pointer'
                }}
            >
